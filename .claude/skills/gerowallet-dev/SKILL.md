...
**1. The two commands CONTRIBUTING.md tells you to run are not gates and never pass.**
On a clean checkout of `development`: `npm run typecheck` exits 2 with ~173 pre-existing errors (tsc resolves neither the unplugin-auto-import globals like `ref`/`computed` nor `.vue` modules; `vue-tsc` has no Vue 2.7 + TS 5.8 build). `npm run lint` exits 1 with 390 errors, all legacy `@typescript-eslint/no-explicit-any`. Both counts drift; never compare absolute numbers between runs, and never read either as a verdict on your change. CONTRIBUTING.md:72-76 and the PR template still say to run them. Do not chase those errors and do not "fix" unrelated files.
...
- **Every new i18n key goes in `us.ts` and `de.ts` in the same commit.** Nothing checks this. A missing German key silently renders English, and a key missing from both renders the raw key text to the user. Warnings are deliberately silenced. See `references/i18n.md`.
