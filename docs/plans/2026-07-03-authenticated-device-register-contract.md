...
2. **Framing gate** (kills COSE malleability / label confusion): require protected-header `alg == EdDSA`, `COSE_Key.kty == OKP`, `crv == Ed25519`, and `COSE_Key.x` is exactly 32 bytes. Require the header `address` text label bytes == the `key_id` bytes (both are written by the signer); reject on disagreement.
3. Extract the signer address bytes from the COSE protected header (`address` text label). Decode → `Cardano.Address`. Resolve to a reward/stake bech32 (already a reward addr here; reject a base-addr header for standard Cardano wallets). Require it **equals `S`** AND equals `proof.stakeAddress` (cross-check the convenience field, no trust).
