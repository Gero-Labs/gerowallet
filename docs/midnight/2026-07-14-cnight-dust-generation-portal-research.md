...
- **Phase 3 (cross-wallet, Midnight side)** ✅ `useDustSources` + `CnightDustSourcesDialog`: from a Midnight wallet, list the same-seed Cardano twin (persisted in `MidnightAddresses`) and every imported Cardano wallet on the anchored network with NIGHT balance (Koios `address_assets`) + registration status (Nexus batch); register or redirect DUST generation to this wallet's own DUST address. Signing is local to the options context (payment + stake witnesses over the locally recomputed tx id); watch-only/hardware rows are read-only. Entry from DustRegistrationDialog.
