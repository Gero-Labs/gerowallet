...
export async function syncDustWalletAndBalanceFees(
  ...
  try {
    ...
    /**
     * Whether this attempt resumed from a LOCAL checkpoint (as opposed to a
     * bootstrap snapshot or a cold start). A checkpoint saved before the
     * wallet's DUST registration relayed contains no dust account, and
     * resuming from it re-persists that same empty state on every subsequent
     * send — so the wallet stays permanently unable to pay its own fees. See
     * the recovery at the fee step below.
     */
    let restoredFromLocalCheckpoint = false;
    ...
    // Bounded. Observed on mainnet 2026-09-07: with no spendable DUST the SDK
    // call never settles and never throws, so the send UI span forever on
    // "Sign transaction" with nothing in the console after the dust
    // checkpoint. A hang is indistinguishable from slowness to the user, so
    // convert it into an error they can act on. The cap is generous — the
    // call itself is local coin selection over already-synced state, not a
    // network round-trip, so anything approaching a minute is a stuck call
    // rather than a slow one.
    let feeBalancingTx: unknown;
    try {
      feeBalancingTx = await withTimeout(
        dustWallet.balanceTransactions(dustSk, txs, args.ttl),
        DUST_FEE_BALANCE_TIMEOUT_MS,
        'Timed out computing the DUST fee for this transaction. This usually '
          + 'means the wallet has no spendable DUST — DUST is generated by '
          + 'holding NIGHT, so an address holding only tokens cannot pay fees.',
      );
    } catch (feeErr) {
      // A checkpoint written before this wallet's DUST registration relayed
      // holds no dust account. Resuming from it looks like a clean, fully
      // synced wallet with nothing to spend — and because the sync path
      // re-persists that state every time, the wallet can never recover on its
      // own. Observed on mainnet: repeated failures from a checkpoint, then an
      // immediate success once the checkpoint was absent and the ledger was
      // bootstrapped fresh.
      //
      // So drop the checkpoint on this failure. The next attempt bootstraps
      // from a Nexus snapshot and replays forward, which is what actually
      // works. Only for a warm restore: cold-started attempts have no poisoned
      // state to blame, and clearing would just force a needless full replay.
      if (shouldDiscardCheckpoint(feeErr, restoredFromLocalCheckpoint)) {
        debugLog('🌙 dust fee failed on a resumed checkpoint — discarding it so the next send re-bootstraps');
        try {
          await clearWalletState(args.sdkNetworkId, 'dust', args.dustSecretSeed);
        } catch { /* best effort: a failed clear only costs another retry */ }
        throw new Error(
          'Could not compute the DUST fee from the saved sync state for this wallet. '
          + 'The saved state has been discarded — please try sending again.',
        );
      }
      throw feeErr;
    }
...
/**
 * How long `dust.balanceTransactions` may run before we call it stuck.
 *
 * It selects DUST coins from state the wallet has already synced — no network
 * round-trip — so this is a stuck-call detector, not a latency allowance.
 */
const DUST_FEE_BALANCE_TIMEOUT_MS = 60_000;

/**
 * Reject with `message` if `promise` hasn't settled within `ms`.
 *
 * The underlying work is NOT cancelled — the SDK gives us no handle to cancel
 * with — so this bounds what the CALLER waits for, and the caller's `finally`
 * still tears the dust wallet down. That is the whole point: an unbounded
 * await here is a permanently spinning send with no error anywhere.
 */
