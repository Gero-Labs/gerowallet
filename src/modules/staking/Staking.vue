          ...
          <v-card-title class="pa-0">
            <v-list-item two-line>
              <v-list-item-content>
                <v-list-item-title style="display: flex">
                  ...
                  <div style="display: flex">
                    ...
                    <v-switch inset dense v-model="isPro" hide-details style="margin-top: 0; align-items: center">
                    </v-switch>
                  </div>
                </v-list-item-title>
              ...
              <v-list-item-action style="align-items: center" class="ma-0" v-if="geroPoolExists && !delegatingToGero">
                <v-card-title style="color: #00dff3; font-size: 18px"> Consider supporting us </v-card-title>
                <v-card-subtitle>
                  <v-btn
                    small
                    style="
          ...
          <v-card-text class="py-0">
            ...
            >
              ...
              <template v-slot:[`item.live_saturation`]="{ item }">
                <v-progress-linear
                  rounded
                  :color="getColor(item.live_saturation)"
                  height="16"
                ...
                <div
                  class="justify-space-between d-flex align-items-center"
                  style="font-size: 10px; text-align-last: justify"
                >
                  <strong>{{
                    item.active_stake
                      | toCurrency(
                  ...
                  <strong
                    ...
                    style="display: inline-flex; font-size: 10px"
                  >
                    <v-icon x-small color="#F97066" style="font-size: 10px; line-height: 1.7"
              ...
              <template v-slot:[`item.fixed_cost`]="{ item }">
                <span style="font-size: 14px; color: white" v-if="loggedWallet"
                  >{{ item.margin + '%' }} /
                  {{
                    item.fixed_cost
                      | toCurrency(false, 0, networks.resolveCurrencySymbol(loggedWallet?.chain, loggedWallet?.network))
                  }}</span
                >
              </template>
                  ...
                  >
                    ...
                    <v-card-text class="pt-0">
                      ...
                      <v-row no-gutters>
                        ...
                        <v-col cols="7">
                          <v-chip
                            x-small
                            color="#085D3A"
                            style="border: 1px solid #75e0a7; color: #75e0a7"
                            v-if="loggedWallet"
                          >
                            {{
                              pool.pledge
                      ...
                      <v-row no-gutters>
                        ...
                        <v-col cols="7">
                          <span style="font-size: 14px; color: white" v-if="pool && loggedWallet"
                            >{{ pool.margin + '%' }} /
                            {{
                              pool.fixed_cost
                                | toCurrency(
...
export default {
  ...
  computed: {
    ...
    ...mapState(useStore, ['pools', 'poolsPagination', 'loggedWallet', 'latestTip', 'baseAddress']),
    ...mapState(walletConfigStore, ['utxos', 'account']),
    numPages() {
      return this.poolsPagination.total_pages || 1;
    },
        stakePools() {
      if (this.pools) {
  ...
  methods: {
    ...mapActions(useStore, ['setStakingProView', 'loadPools']),
    getColor(value) {
      if (value > 100) {
        value = 100;
      }
    ...
    delegate(row) {
      ...
      const delegationCertificate = Certificate.new_stake_delegation(
        StakeDelegation.new(
          Credential.from_keyhash(Ed25519KeyHash.from_hex(wallet.stakeKey().hash().hex())),
          Ed25519KeyHash.from_bech32(this.selectedPool.pool_id_bech32)
        )
      );
    ...
    async loadPoolsWithPagination() {
      console.log('loadPoolsWithPagination', this.page, this.pageSize, this.search, this.hideSaturated, this.pledgeMet);
      this.loading = true;
      try {
        const params = {
          page: this.page,
        ...
        if (this.hideSaturated) {
          params.hide_saturated = this.hideSaturated;
        }
        if (this.pledgeMet) {
          params.pledge_met = this.pledgeMet;
        }

        await this.loadPools(params);
      } catch (error) {
        console.error('Error loading pools with pagination:', error);
      } finally {
        this.loading = false;
      }
    },
    ...
    async handlePageChange(newPage) {
      this.page = newPage;
      await this.loadPoolsWithPagination();
    },
